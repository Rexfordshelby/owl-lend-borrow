...
ALTER TYPE item_category ADD VALUE 'services';

-- Drop the problematic condition constraint
ALTER TABLE public.items DROP CONSTRAINT IF EXISTS items_condition_check;

-- Create condition enum type if it doesn't exist
DO $$
BEGIN
...
-- Create function to calculate total cost based on service/item type
